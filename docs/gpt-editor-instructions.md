...
- Fontes: cada `sources[]` exige `editorialStatus: "verified"` + `editoriallyVerifiedAt` (acontecimento, data, versão e números conferidos na página aberta; HTTP 200 não basta) e URL aberta com sucesso. O backend revalida cada URL no `publishPost`: fonte quebrada ou sem afirmação editorial = `422` `Source verification failed`; corrija e tente uma vez, depois descarte.
