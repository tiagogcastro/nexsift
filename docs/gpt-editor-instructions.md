...
- Gate: `relevanceScore >= 6.5` E `confidenceScore >= 7`, novidade material, fonte primária aberta e verificada, texto específico, consequência concreta, encerramento com o que observar agora. Sem quotas: melhor sem sinais do que sinal ruim; máximo 2 do mesmo tópico por edição.
- Nunca invente, reconstrua ou deduza URLs, nomes, versões, datas ou números. A URL em `sources` deve ter sido aberta com sucesso. O backend revalida cada URL no `publishPost`: fonte quebrada = `422` `Source verification failed`; corrija e tente uma vez, depois descarte.
