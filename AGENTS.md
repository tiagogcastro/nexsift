...
- Slug is derived as `{topic}-{slugified title, max 40 chars}-{signalDate}`; republishing the same slug updates the signal. Each signal has exactly one `topic`; `relatedTopics` are contextual metadata only and never place a signal on a topic page.
